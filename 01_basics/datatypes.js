...
// false because symbols are unique and they are not equal even if they have the same description.
console.log(sym1 == sym2);
// false because symbols are unique and they are not equal even if they have the same description.
let sym3 = Symbol("sym2")
// it is not the same as sym2 and sym3 because it has a different description but it is still a unique symbol and it is not equal to sym1 and sym2.

let sym4 = Symbol.for("sym1")
let sym5 = Symbol.for("sym1")
console.log(sym4 === sym5); 
// true because Symbol.for() creates a symbol in the global symbol registry and it returns the same symbol for the same key, so sym4 and sym5 are the same symbol because they have the same key "sym1".
console.log(sym4 == sym5);
// true because Symbol.for() creates a symbol in the global symbol registry and it returns the same symbol for the same key, so sym4 and sym5 are the same symbol because they have the same key "sym1".
